...
// Product names that look like a camelCase symbol or a source file.
const NOT_CODE_WORDS = [
  /\b(?:macOS|iOS|iPadOS|eBay|iPhone|iPad)\b/g,
  /\b(?:node|vue|next|nuxt|express|three|d3|chart|ember|deno|socket\.io)\.js\b/gi
];
...
export function isCodeQuestion(prompt) {
  ...
  const codeText = NOT_CODE_WORDS.reduce((t, re) => t.replace(re, ""), text);
  if (STRONG.some(re => re.test(codeText))) return true;
